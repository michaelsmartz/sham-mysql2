...
<div class="form-group col-xs-6 {{ $errors->has('total') ? 'has-error' : '' }}">
    <label for="total">Total</label>
        <input class="form-control" name="total" type="number" id="total" min="0" {!! (!is_null($employee_id) && $employee_id == optional($data)->employee_id) ?'disabled':'' !!} value="{{ old('total', optional($entitlement)->total) }}" placeholder="Enter total here..." step="0.01" required="true">
        {!! $errors->first('total', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group col-xs-6 {{ $errors->has('taken') ? 'has-error' : '' }}">
    <label for="taken">Taken</label>
        <input class="form-control" name="taken" type="number" id="taken" min="0" {!! (!is_null($employee_id) && $employee_id == optional($data)->employee_id) ?'disabled':'' !!} value="{{ old('taken', optional($entitlement)->taken) }}" placeholder="Enter taken here..." step="0.01" required="true">
