...
<div class="form-group col-xs-12 {{ $errors->has('username') ? 'has-error' : '' }}">
    <label for="username">Username</label>
        <input class="form-control" name="username" type="text" id="username" value="{{ old('username', optional($user)->username) }}" minlength="1" required="true" placeholder="Enter username">
        {!! $errors->first('username', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group col-xs-12 {{ $errors->has('password') ? 'has-error' : '' }}">
    <label for="password">Password</label>
        <input class="form-control" name="password" type="password" id="password" value="{{ old('password', optional($user)->password) }}" placeholder="Enter password">
        {!! $errors->first('password', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group col-xs-12 {{ $errors->has('email') ? 'has-error' : '' }}">
    <label for="email">Email</label>
        <input class="form-control" name="email" type="email" id="email" value="{{ old('email', optional($user)->email) }}" placeholder="Enter email">
        {!! $errors->first('email', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group col-xs-12 {{ $errors->has('cell_number') ? 'has-error' : '' }}">
    <label for="cell_number">Cell Number</label>
        <input class="form-control" name="cell_number" type="number" id="cell_number" value="{{ old('cell_number', optional($user)->cell_number) }}" placeholder="Enter cell number">
...
<div class="form-group col-xs-12 {{ $errors->has('silence_start') ? 'has-error' : '' }}">
    <label for="silence_start">Silence Start</label>
        <input class="form-control timepicker" name="silence_start" type="text" id="silence_start" value="{{ old('silence_start', optional($user)->silence_start) }}" minlength="1" placeholder="Enter silence start">
...
<div class="form-group col-xs-12 {{ $errors->has('silence_end') ? 'has-error' : '' }}">
    <label for="silence_end">Silence End</label>
        <input class="form-control timepicker" name="silence_end" type="text" id="silence_end" value="{{ old('silence_end', optional($user)->silence_end) }}" minlength="1" placeholder="Enter silence end">
    ...
    <label for="sham_user_profile_id">Sham User Profile</label>
        <select class="form-control" id="sham_user_profile_id" name="sham_user_profile_id" required="true">
        	...
        	@foreach ($sham_user_profile_ids as $key => $sham_user_profile_id)
			    <option value="{{ $key }}" {{ old('sham_user_profile_id', optional($user)->sham_user_profile_id) == $key ? 'selected' : '' }}>
...
<div class="form-group col-xs-12 {{ $errors->has('employee_id') ? 'has-error' : '' }}">
    <label for="employee_id">Employee</label>
        <select class="form-control" id="employee_id" name="employee_id" required="true">
        	    <option value="" style="display: none;" {{ old('employee_id', optional($user)->employee_id ?: '') == '' ? 'selected' : '' }} disabled selected>Select Employee</option>
        	@foreach ($employee_ids as $key => $employee_id)
			    <option value="{{ $key }}" {{ old('employee_id', optional($user)->employee_id) == $key ? 'selected' : '' }}>
			    	{{ $employee_id }}
			    </option>
			@endforeach
        </select>
