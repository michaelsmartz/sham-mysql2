<div class="row">
    <div class="form-group col-xs-6 {{ $errors->has('username') ? 'has-error' : '' }}">
        <label for="username">Username</label>
            <input class="form-control" name="username" type="text" id="username" value="{{ old('username', optional($data)->username) }}" minlength="1" required="true" placeholder="Enter username" {!! (isset($mode) && $mode =='edit')?'disabled':'' !!}>
    ...
    <div class="form-group col-xs-6 {{ $errors->has('email') ? 'has-error' : '' }}">
        <label for="email">Email</label>
            <input class="form-control" name="email" type="email" id="email" value="{{ old('email', optional($data)->email) }}" placeholder="Enter email" {!! (isset($mode) && $mode =='edit')?'disabled':'' !!}>
